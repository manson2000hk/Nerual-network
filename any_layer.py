...
###################  Testing Accurancy (for 1 layer, 1 nerual)  ################################
...
###########    Testing Accurancy (For any layer and neural)   ################

t_layer_out = []
for o in range(test_in.shape[0]):
    t_inp = test_in[o]
    for r in range(n_cal):
        
        t_out = forward(t_inp , name_w[r] , name_b[r])
        t_inp = t_out
        if r == n_cal-1:
            
            t_layer_out.append(t_out)
        


total_test_error = 0

for s in range(len(t_layer_out)):
    
    t_error = (test_out[s] - t_layer_out[s]) ** 2
    # print(test_out[s] , t_layer_out[0][s] , t_error)
    total_test_error += t_error

testing_out = []
for t in t_layer_out:
    testing_out.append(t[0][0])

testing_output = np.array([testing_out])

print("\n" , "Testing Output = " , "\n" , testing_out , "\n")
print("\n" , "Total Testing error = " , "\n" ,math.sqrt(total_test_error))
print("\n" , "Duration : " , "\n" ,round((time.time() - Start_T) , 4) , "s")
